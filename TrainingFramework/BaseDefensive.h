...
{
public:
	BaseDefensive() :AbleToAttack(), Sprite2D(), AttackAble(), m_level(0), m_maxlevel(3), m_totalCost(0), m_disposable(false), m_type(TowerType::Spot) {}
	BaseDefensive(std::shared_ptr<Model> model, std::shared_ptr<Shader> shader, std::shared_ptr<Texture> texture,
		float x, float y, int iwidth, int iheight, int width, int height, float range, float attackSpeed, float damage, int level, TowerType type, int maxlevel)
		:Sprite2D(model, shader, texture, x, y, iwidth, iheight, width, height), AttackAble(),
		AbleToAttack(range, attackSpeed, damage), m_level(level), m_disposable(false), m_type(type), m_maxlevel(maxlevel) {
	}
	BaseDefensive(std::shared_ptr<Model> model, std::shared_ptr<Shader> shader, std::shared_ptr<Texture> texture,
		float x, float y, int iwidth, int iheight, int width, int height, float range, float attackSpeed, float damage, int level, TowerType type, int maxlevel, float hitPoint, float maxHitPoint)
	...
	void HandleTouchEvents(GLint x, GLint y, bool bIsPressed, std::shared_ptr<BaseDefensive> tower) { m_towerOption->HandleTouchEvents(x, y, bIsPressed, tower); };
